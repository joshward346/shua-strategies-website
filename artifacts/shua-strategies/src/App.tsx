...
const CARS_GUIDE_FORM_URL = 'https://forms.gle/EShunvhfFPTSnsUKA';
    ...
    <div className="container-wide">
      <div className="footer-grid">
        ...
        <div><h3>Stay close</h3><div className="footer-links">
          ...
          <a href="https://www.instagram.com/shuastrategies/" target="_blank" rel="noreferrer" data-testid="link-footer-instagram"><Instagram size={13} /> Instagram</a>
        </div></div>
      </div>
...
function ResourceCard({ number, title, copy, featured = false, formUrl }: { number: string; title: string; copy: string; featured?: boolean; formUrl?: string }) {
  const [open, setOpen] = useState(false);
  return <article className={`resource-card reveal ${featured ? 'featured' : ''}`} data-testid={`card-resource-${number}`}><div className="resource-type">{number}</div><h3>{title}</h3><p>{copy}</p>{open ? <div className="resource-gate"><EmailGate title={title} formUrl={formUrl} /></div> : <button type="button" className="text-link" onClick={() => setOpen(true)} data-testid={`button-resource-${number}`}>Get the resource <ArrowUpRight size={14} /></button>}</article>;
}
