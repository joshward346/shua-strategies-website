...
import { ArrowUpRight, Check, CheckCircle2, Clock3, FileText, Instagram, Linkedin, Mail, Menu, MoveRight, ShieldCheck, Target, Users, X } from 'lucide-react';
    ...
    <header className="topbar">
      <div className="container-wide">
        <div className="nav-inner">
          ...
          <Link href="/book" className="nav-cta" data-testid="link-nav-book">Book a free 15-minute call <ArrowUpRight size={14} /></Link>
        ...
        {open && <nav className="mobile-panel" aria-label="Mobile navigation">
          ...
          <Link href="/book" className="nav-cta" onClick={() => setOpen(false)} data-testid="link-mobile-book">Book a free 15-minute call <ArrowUpRight size={14} /></Link>
        </nav>}
      </div>
    </header>
  );
...
function CTA({ children = 'Book a free 15-minute call', href = '/book' }: { children?: ReactNode; href?: string }) {
...
function OfferRow({ number, name, price, copy, scope, action, href = '/book', onCheckout }: { number: string; name: string; price: string; copy: string; scope: string; action: string; href?: string; onCheckout?: () => void }) {
  return <article className="offer-row reveal" data-testid={`card-offer-${number}`}>
    ...
    <div className="offer-action"><p className="offer-price">{price}</p>{onCheckout ? <button type="button" className="text-link" onClick={onCheckout} data-testid={`button-stripe-${number}`}>{action} <ArrowUpRight size={14} /></button> : <Link href={href} className="text-link" data-testid={`link-offer-${number}`}>{action} <ArrowUpRight size={14} /></Link>}</div>
  </article>;
}
...
function StripePlaceholder({ name, price, action }: { name: string; price: string; action: string }) {
  const [requested, setRequested] = useState(false);
  return <div className="payment-area">
    <div className="stripe-note">{requested ? 'Stripe payment link placeholder selected — connect your live checkout link here.' : `STRIPE PLACEHOLDER · ${name} · ${price}`}</div>
    <button type="button" className="btn-primary" style={{ marginTop: 18 }} onClick={() => setRequested(true)} data-testid={`button-stripe-checkout-${name.toLowerCase().replaceAll(' ', '-')}`}>{requested ? 'Payment link noted' : action} <ArrowUpRight size={15} /></button>
  </div>;
}

function EmailGate({ title }: { title: string }) {
...
function Home() {
  ...
  return <Shell><main>
    <section className="hero"><div className="container-wide hero-grid">
      ...
      <div className="hero-art reveal delay-2" aria-label="Founder headshot placeholder"><div className="floating-card top"><div className="float-label">Personal track record</div><div className="float-value">99th percentile CARS</div></div><div className="portrait-placeholder"><span className="portrait-label">Joshua Ward, MD · founder</span></div><div className="floating-card bottom"><div className="float-label">The working principle</div><div className="float-value">Strategy before volume</div></div></div>
    </div></section>
    <TrustStrip />
    <section className="section"><div className="container-wide"><div className="split-heading reveal"><Eyebrow>The strategic layer</Eyebrow><div><p className="manifesto">The resources are not the problem. It’s knowing what to do with them — <em>and why.</em></p><p className="section-intro">UWorld, Anki, AAMC materials, prep books: ambitious students usually have plenty of resources. Shua Strategies helps diagnose the real constraint, build a personal sequence, and coach the execution.</p></div></div><div className="principles">{[['01', 'Diagnose the signal', 'A score, transcript, or draft is information — not a verdict.'], ['02', 'Sequence the work', 'Put the right resource in the right place, instead of adding another tab.'], ['03', 'Build a repeatable rhythm', 'A plan has to survive an ordinary Tuesday, not just a burst of motivation.']].map(([number, title, copy], index) => <div className={`principle reveal delay-${index + 1}`} key={number} data-testid={`card-principle-${number}`}><span className="principle-number">{number}</span><h3>{title}</h3><p>{copy}</p></div>)}</div></div></section>
...
function Mcat() {
  ...
  return <Shell><main>
    <section className="page-hero"><div className="container-wide page-hero-grid"><div className="reveal"><Eyebrow>MCAT preparation</Eyebrow><h1 className="display-lg" style={{ marginTop: 21 }}>Stop outthinking <span className="accent-italic">CARS.</span></h1><p className="body-lg muted">Build a study strategy around your actual performance — not a generic calendar or another pile of resources.</p></div><div className="page-stamp reveal delay-2">READ<br />WITH<br />INTENT</div></div></section>
     <section className="section"><div className="container-wide price-page"><div className="price-intro reveal"><Eyebrow>Two ways to begin</Eyebrow><h2>Strategy first.<br /><span className="accent-italic">Then repetition.</span></h2><p>Joshua earned a 99th percentile MCAT CARS score. His approach starts with a close look at how you are currently working, then builds the smallest useful system for the time you have left.</p><div className="scope-list"><li><b>01</b> Current performance analysis</li><li><b>02</b> Content-gap assessment</li><li><b>03</b> Resource sequencing and full-length schedule</li><li><b>04</b> CARS strategy</li><li><b>05</b> Written study plan</li></div></div><div className="reveal delay-1"><div className="price-box"><h3>MCAT Strategy Intensive</h3><div className="large-price">$600</div><p className="muted">A deep diagnostic and personalized MCAT roadmap. Leave knowing exactly what to do between now and test day.</p><div className="detail-list"><div><Check size={15} />Current performance analysis</div><div><Check size={15} />Content-gap assessment</div><div><Check size={15} />Resource sequencing</div><div><Check size={15} />CARS strategy</div><div><Check size={15} />Full-length exam schedule</div><div><Check size={15} />Written study plan</div></div><StripePlaceholder name="MCAT Strategy Intensive" price="$600" action="Book Your Strategy Intensive" /></div><div style={{ height: 16 }} /><div className="price-box"><h3>MCAT Private Coaching</h3><div className="large-price">$2,000</div><p className="muted">10 hours of individualized coaching built on top of your strategy: CARS coaching, content remediation, full-length analysis, and accountability.</p><div className="detail-list"><div><Check size={15} />Performance coaching, not a stack of tutoring hours</div><div><Check size={15} />CARS coaching and content remediation</div><div><Check size={15} />Full-length analysis</div><div><Check size={15} />Accountability between sessions</div></div><StripePlaceholder name="MCAT Private Coaching" price="$2,000" action="Reserve Private Coaching" /></div></div></div></section>
...
function FuturePhysician() {
  ...
  return <Shell><main>
    ...
    <section id="application" className="section"><div className="container-wide booking-grid"><div className="booking-aside reveal"><Eyebrow>Application required</Eyebrow><h2>Tell Joshua where you are headed.</h2><p>Share a little context. Joshua reviews applications personally; qualified applicants are invited to a private consultation, then enrollment.</p><ul className="promise-list"><li><Users size={15} />Limited to 5 active clients at a time</li><li><FileText size={15} />Short application, no performance</li><li><ShieldCheck size={15} />Private consultation before enrollment</li></ul></div><div className="booking-card reveal delay-1"><h2>Future Physician application</h2><p>This is an application, not a payment form.</p>{applied ? <div className="booking-success" data-testid="status-future-application"><CheckCircle2 size={21} style={{ color: 'var(--royal)' }} /><h3>Thank you, {name.split(' ')[0] || 'there'}.</h3><p>Your application is ready for Joshua’s review. Contact and Calendly integrations will be connected here.</p><button type="button" className="btn-secondary" onClick={() => setApplied(false)} data-testid="button-future-application-reset">Edit application</button></div> : <form className="booking-form" onSubmit={(event) => { event.preventDefault(); if (name.trim() && email.trim() && context.trim()) setApplied(true); }}><label className="form-label" htmlFor="future-name">Name<input id="future-name" className="field" required value={name} onChange={(event) => setName(event.target.value)} data-testid="input-future-name" /></label><label className="form-label" htmlFor="future-email">Email<input id="future-email" className="field" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} data-testid="input-future-email" /></label><label className="form-label" htmlFor="future-context">Where are you in your premed journey?<textarea id="future-context" className="field form-textarea" required value={context} onChange={(event) => setContext(event.target.value)} data-testid="input-future-context" /></label><button className="btn-primary" type="submit" data-testid="button-future-application-submit">Submit application <ArrowUpRight size={15} /></button></form>}</div></div></section>
  </main></Shell>;
}
...
function ResourceCard({ number, title, copy, featured = false }: { number: string; title: string; copy: string; featured?: boolean }) {
  const [open, setOpen] = useState(false);
  return <article className={`resource-card reveal ${featured ? 'featured' : ''}`} data-testid={`card-resource-${number}`}><div className="resource-type">{number}</div><h3>{title}</h3><p>{copy}</p>{open ? <div className="resource-gate"><EmailGate title={title} /></div> : <button type="button" className="text-link" onClick={() => setOpen(true)} data-testid={`button-resource-${number}`}>Get the resource <ArrowUpRight size={14} /></button>}</article>;
}
...
function Resources() {
  ...
  return <Shell><main>
    ...
    <section className="section"><div className="container-wide"><div className="split-heading reveal"><Eyebrow>The resource shelf</Eyebrow><div><p className="manifesto">Start with the one you’ll use today.</p><p className="section-intro">Each resource is designed to replace one kind of overwhelm with one useful move.</p></div></div><div className="resource-grid"><ResourceCard number="01 · CARS" featured title="CARS Strategy Guide" copy="A grounded guide to reading passages with purpose, learning from missed questions, and making your next CARS session count." /><ResourceCard number="02 · BIO / BIOCHEM" title="High-Yield Biology/Biochemistry Sheet" copy="A compact review companion for concepts that show up often and connect across the exam." /><ResourceCard number="03 · PSYCH / SOC" title="High-Yield Psychology/Sociology Sheet" copy="A quick-reference guide for terms, theories, and distinctions worth keeping close." /><ResourceCard number="04 · CHEM / PHYSICS" title="High-Yield Chemistry/Physics Sheet" copy="A practical refresher for equations, relationships, and problem-solving patterns." /><ResourceCard number="05 · ORGANIC CHEMISTRY" title="High-Yield Organic Chemistry Sheet" copy="A concise review of reactions and concepts to revisit with intention." /></div></div></section>
    <section className="section section-soft"><div className="container-wide"><div className="newsletter reveal"><div><Eyebrow>The occasional note</Eyebrow><h2>A little clarity,<br /><span className="accent-italic">in your inbox.</span></h2><p className="muted" style={{ fontSize: 13, lineHeight: 1.6 }}>Short notes on studying, storytelling, and staying human on the road to medicine.</p></div><div>{sent ? <p className="form-message" data-testid="status-newsletter-success">You’re on the list. MailerLite delivery will be connected here.</p> : <form className="newsletter-form" onSubmit={(event) => { event.preventDefault(); if (email.trim()) setSent(true); }}><label className="sr-only" htmlFor="newsletter-email">Email address</label><input id="newsletter-email" className="field" type="email" required placeholder="you@example.com" value={email} onChange={(event) => setEmail(event.target.value)} data-testid="input-newsletter-email" /><button className="btn-primary" type="submit" data-testid="button-newsletter-submit">Join the list <ArrowUpRight size={14} /></button></form>}<p className="placeholder-note">MAILERLITE EMBED PLACEHOLDER · replace this form with approved embed</p></div></div></div></section>
  </main></Shell>;
}
...
function Book() {
  ...
  return <Shell><main>
    ...
    <section className="section section-soft"><div className="container-narrow"><div className="embed-box reveal"><strong>CALENDLY EMBED PLACEHOLDER</strong> Paste the approved Calendly inline embed here after the contact flow is configured. This page is for the free 15-minute consultation only.</div></div></section>
  </main></Shell>;
}
