...
function Results() {
  usePageMeta('Results and experience', 'A transparent look at Joshua Ward, MD’s personal track record across MCAT coaching and admissions advising.');
  const medSchools = ['NYU Grossman School of Medicine', 'Washington University School of Medicine in St. Louis', 'Icahn School of Medicine at Mount Sinai', 'Albert Einstein College of Medicine', 'UT Southwestern Medical School', 'George Washington University School of Medicine', 'University of New Mexico BA/MD Program'];
  const undergradSchools = ['Cornell University', 'Georgetown University', 'Tufts University', 'Carleton College', 'Tulane University', 'Macalester College', 'Occidental College', 'Pitzer College', 'Scripps College'];
  const neutralTagStyle = { borderColor: 'var(--line)', color: 'hsl(var(--foreground))', fontWeight: 400 };
  return <Shell><main>
    <section className="page-hero"><div className="container-wide page-hero-grid"><div className="reveal"><Eyebrow>The track record</Eyebrow><h1 className="display-lg" style={{ marginTop: 21 }}>Personal track record.<br /><span className="accent-italic">No theater.</span></h1><p className="body-lg muted">More than five years of working directly with students across MCAT preparation, college admissions, and medical school admissions.</p></div><div className="page-stamp reveal delay-2">5+ YEARS<br />40–50+ STUDENTS<br />ONE-ON-ONE WORK</div></div></section>
    <section className="section"><div className="container-wide"><div className="trust-grid reveal" style={{ borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)', padding: '24px 0' }}>{[['5+ years', 'Admissions advising & MCAT coaching'], ['40–50+', 'Students personally advised and mentored'], ['515+', 'Multiple students have scored 515 or higher on the MCAT'], ['99th percentile', 'Joshua’s personal CARS score']].map(([value, label], index) => <div className="trust-item" key={value} data-testid={`text-results-stat-${index}`}><div className="trust-value">{value}</div><div className="trust-label">{label}</div></div>)}</div></div></section>
    <section className="section section-soft"><div className="container-wide"><div className="split-heading reveal"><Eyebrow>MCAT results</Eyebrow><div><p className="manifesto">Real scores.<br />Real students.</p><p className="section-intro">Students have come to me mid-preparation with stalled scores and left with results in the 515 to 520s range. Others have started from an earlier point and built toward a strong first attempt.</p><p className="section-intro">The work is different for every student. The goal is always the same.</p></div></div></div></section>
    <section className="section"><div className="container-narrow" style={{ textAlign: 'center' }}>
      <div className="reveal"><h2 className="display-md">Acceptances.</h2><p className="muted" style={{ margin: '20px auto 0', maxWidth: 520, lineHeight: 1.7 }}>Students Joshua has personally advised and mentored have gone on to earn acceptances at institutions including:</p></div>
      <div className="reveal delay-1" style={{ marginTop: 54 }}>
        <Eyebrow>Medical schools</Eyebrow>
        <div className="school-list" style={{ justifyContent: 'center', marginTop: 18 }}>{medSchools.map((school, index) => <span key={school} style={index === medSchools.length - 1 ? neutralTagStyle : undefined}>{school}</span>)}</div>
        <p className="muted" style={{ marginTop: 12, fontSize: 12 }}>and more.</p>
      </div>
      <div className="reveal delay-2" style={{ marginTop: 46 }}>
        <Eyebrow>Undergraduate institutions</Eyebrow>
        <div className="school-list" style={{ justifyContent: 'center', marginTop: 18 }}>{undergradSchools.map((school, index) => <span key={school} style={index === undergradSchools.length - 1 ? neutralTagStyle : undefined}>{school}</span>)}</div>
        <p className="muted" style={{ marginTop: 12, fontSize: 12 }}>and more.</p>
      </div>
      <p className="results-disclaimer" style={{ maxWidth: 620, margin: '40px auto 0' }}>These results reflect students Joshua personally advised or mentored across paid advising, volunteer mentorship, and focused application support. This list is not exhaustive. Individual outcomes vary.</p>
    </div></section>
    <section className="section section-soft"><div className="container-narrow" style={{ textAlign: 'center' }}><div className="reveal"><Eyebrow>Ready to work together?</Eyebrow><h2 className="display-md" style={{ marginTop: 18 }}>Your application is still yours.<br /><span className="accent-italic">You don’t have to build it alone.</span></h2><p className="muted" style={{ margin: '22px auto 29px', maxWidth: 480, lineHeight: 1.7 }}>Whether you’re preparing for the MCAT, applying this cycle, or thinking several years ahead, we can start by figuring out what would make the biggest difference now.</p><CTA>Book a free 15-minute call</CTA></div></div></section>
  </main></Shell>;
}
